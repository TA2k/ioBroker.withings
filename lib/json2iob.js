      ...
      objectKeys.forEach(async (key) => {
        ...
        if (key === "hr" || key === "rr" || key === "snoring") {
          if (typeof element[key] === "object") {
            element[key] = Object.entries(element[key]).map(([key, value]) => ({ timestamp: value.timestamp, value: value.value }));
          }
        }
        if (Array.isArray(element[key])) {
          ...
          if (!this.alreadyCreatedObjects[path + "." + key]) {
            ...
            //custom
            if (key.endsWith("time") || key.endsWith("date") || key.endsWith("time") || key.endsWith("created")) {
